...
* 改进了 iSCSI 启动程序名称的解析。（问题描述 [#681]
* 修复了不允许使用 CSI 存储类参数的问题描述 。（问题描述 ##598]
* 修复了 Trident CRD 中的重复密钥声明。（问题描述 [#671]
* 修复了不准确的 CSI Snapshot 日志。（问题描述 [#629]
* 修复了已删除节点上的卷已取消发布的问题描述 。（问题描述 [#691]
* 增加了对块设备上文件系统不一致问题的处理。（问题描述 （ #656 ）
* 修复了在安装期间设置 `imageRegistry` 标志时问题描述 提取自动支持映像的问题。（问题描述 （ #715 ）
...
* 现在，与 Trident 安全端点的入站连接至少需要 TLS 1.3 。（问题描述 （第 698 号）
* 现在， Trident 会将 HSTS 标头添加到其安全端点的响应中。
* Trident 现在会尝试自动启用 Azure NetApp Files UNIX 权限功能。
* * Kubernetes * ： Trident demonset 现在以 system-node-critical 优先级类运行。（问题描述 [#694/]
